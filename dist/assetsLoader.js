(function (global, factory) {
  ...
  (global = global || self, global.assetsLoader = factory());
   ...
   * @param options 配置
   */
  var getRandomTime = function (count, options) {
  ...
  var asstesLoader = function (assets, options) {
      ...
      bundler.push.apply(bundler, assets.map(function (url) {
          switch (matchType(url)) {
              case 'image':
                  return Loader(url, { success: function () { return emitter.emit('load'); } });
              case 'unknow':
                  return Unknow(url, { success: function () {
                          console.warn("Assets Loader: unknow asset type " + url);
                          emitter.emit('load');
                      } });
          }
      }));
      bundler.push.apply(bundler, getRandomTime(random, {
